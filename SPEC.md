...
27. **Trips are ordered most recent first** in both views: the newest trip is the top card of
    ...
    drawn however many trips there are. Travelling by the dots lifts the card out of the pack
    rather than sliding it in from the side: it starts lying in the second slot, drawn under the
    card that was covering it, and rises into place, coming out in front only once it has
    settled. With the motion effect off it simply changes.
